...
export const Typer: React.FC<TyperProps> = ({
  ...
  useEffect(() => {
    if (typerRef.current && blinkerRef.current && texts.length > 0 && !isRunningRef.current) {
      isRunningRef.current = true;
      // Clear any existing content
      typerRef.current.textContent = "";
      
      typeOut(typerRef.current, blinkerRef.current, texts, speed, delay)
        .finally(() => {
  ...
  return (
    <>
      ...
      <span ref={blinkerRef} className={`blinker inline-block text-amber-500 ${blinkerClass}`}>|</span>
    </>
  );
}; 