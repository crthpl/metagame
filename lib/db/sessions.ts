...
export const sessionsService = {
  ...
  sessionIsFull: async ({sessionId}: {sessionId: string}) => {
    ...
    return session.max_capacity !== null && (session.rsvp_count || 0) >= session.max_capacity
  },
  ...
  unrsvpUserFromSession: async ({sessionId, userId}: {sessionId: string, userId: string}) => {
    const supabase = createServiceClient()
    const { data, error } = await supabase
      .from('session_rsvps')
      .delete()
  ...
  userHasRsvp: async ({sessionId, userId}: {sessionId: string, userId: string}) => {
    const supabase = createServiceClient()
    const { data, error } = await supabase
      .from('session_rsvps')
      .select()
  ...
  toggleUserRsvpForSession: async ({sessionId, userId}: {sessionId: string, userId: string}) => {
  ...
  rsvpUserToSession: async ({sessionId, userId}: {sessionId: string, userId: string}) => {
    const supabase = createServiceClient()
    ...
    const { data, error } = await supabase
      .from('session_rsvps')
      .insert({ user_id: userId, session_id: sessionId, on_waitlist: sessionFull })
      .select()
    if (error) {
      throw new Error(error.message)
    }
  ...
  unrsvpUserFromAllSessions: async ({userId}: {userId: string}) => {
    ...
    for (const rsvp of userRsvps) {
      await sessionsService.unrsvpUserFromSession({sessionId: rsvp.session_id, userId})
    }
  },
